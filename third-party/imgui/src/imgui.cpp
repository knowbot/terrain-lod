...
{
    ImU32 crc = ~seed;
    const unsigned char* tex_data = (const unsigned char*)data_p;
    const ImU32* crc32_lut = GCrc32LookupTable;
    while (data_size-- != 0)
        crc = (crc >> 8) ^ crc32_lut[(crc & 0xFF) ^ *tex_data++];
    return ~crc;
}
...
ImU32 ImHashStr(const char* data_p, size_t data_size, ImU32 seed)
{
    ...
    if (data_size != 0)
    {
        while (data_size-- != 0)
        {
            unsigned char c = *tex_data++;
            if (c == '#' && data_size >= 2 && tex_data[0] == '#' && tex_data[1] == '#')
                crc = seed;
    ...
    {
        while (unsigned char c = *tex_data++)
        {
            if (c == '#' && tex_data[0] == '#' && tex_data[1] == '#')
                crc = seed;
            crc = (crc >> 8) ^ crc32_lut[(crc & 0xFF) ^ c];
        }
    }
...
ImU64   ImFileRead(void* tex_data, ImU64 sz, ImU64 count, ImFileHandle f)           { return fread(tex_data, (size_t)sz, (size_t)count, f); }
ImU64   ImFileWrite(const void* tex_data, ImU64 sz, ImU64 count, ImFileHandle f)    { return fwrite(tex_data, (size_t)sz, (size_t)count, f); }
...
static ImGuiStorage::ImGuiStoragePair* LowerBound(ImVector<ImGuiStorage::ImGuiStoragePair>& tex_data, ImGuiID key)
{
    ImGuiStorage::ImGuiStoragePair* first = tex_data.Data;
    ImGuiStorage::ImGuiStoragePair* last = tex_data.Data + tex_data.Size;
...
void ImGui::PopStyleVar(int count)
{
    ...
    while (count > 0)
    {
        ...
        void* tex_data = info->GetVarPtr(&g.Style);
        if (info->Type == ImGuiDataType_Float && info->Count == 1)      { ((float*)tex_data)[0] = backup.BackupFloat[0]; }
        else if (info->Type == ImGuiDataType_Float && info->Count == 2) { ((float*)tex_data)[0] = backup.BackupFloat[0]; ((float*)tex_data)[1] = backup.BackupFloat[1]; }
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)
    {
        ...
        {
            ImGuiSizeCallbackData tex_data;
            tex_data.UserData = g.NextWindowData.SizeCallbackUserData;
            tex_data.Pos = window->Pos;
            tex_data.CurrentSize = window->SizeFull;
            tex_data.DesiredSize = new_size;
            g.NextWindowData.SizeCallback(&tex_data);
            new_size = tex_data.DesiredSize;
        }
...
bool ImGui::SetDragDropPayload(const char* type, const void* tex_data, size_t data_size, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    ImGuiPayload& payload = g.DragDropPayload;
    if (cond == 0)
    ...
    IM_ASSERT((tex_data != NULL && data_size > 0) || (tex_data == NULL && data_size == 0));
    ...
    if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
    {
        ...
        if (data_size > sizeof(g.DragDropPayloadBufLocal))
        {
            ...
            memcpy(payload.Data, tex_data, data_size);
        }
        else if (data_size > 0)
        {
            // Store locally
