...
static bool Items_ArrayGetter(void* tex_data, int idx, const char** out_text)
{
    const char* const* items = (const char* const*)tex_data;
    if (out_text)
        *out_text = items[idx];
    return true;
}
...
static bool Items_SingleStringGetter(void* tex_data, int idx, const char** out_text)
{
    // FIXME-OPT: we could pre-compute the indices to fasten this. But only 1 active combo means the waste is limited.
    const char* items_separated_by_zeros = (const char*)tex_data;
    int items_count = 0;
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* tex_data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    if (*current_item >= 0 && *current_item < items_count)
        items_getter(tex_data, *current_item, &preview_value);
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (!items_getter(tex_data, i, &item_text))
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* tex_data, int items_count, int height_in_items)
{
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* tex_data, int idx), void* tex_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (scale_min == FLT_MAX || scale_max == FLT_MAX)
    {
        ...
        {
            const float v = values_getter(tex_data, i);
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            const float v0 = values_getter(tex_data, (v_idx + values_offset) % values_count);
            const float v1 = values_getter(tex_data, (v_idx + 1 + values_offset) % values_count);
        ...
        float v0 = values_getter(tex_data, (0 + values_offset) % values_count);
        float t0 = 0.0f;
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            const float v1 = values_getter(tex_data, (v1_idx + values_offset + 1) % values_count);
...
static float Plot_ArrayGetter(void* tex_data, int idx)
{
    ImGuiPlotArrayGetterData* plot_data = (ImGuiPlotArrayGetterData*)tex_data;
...
{
    ImGuiPlotArrayGetterData tex_data(values, stride);
    PlotEx(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void*)&tex_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotLines(const char* label, float (*values_getter)(void* tex_data, int idx), void* tex_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    PlotEx(ImGuiPlotType_Lines, label, values_getter, tex_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotHistogram(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
    ImGuiPlotArrayGetterData tex_data(values, stride);
    PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&tex_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}

void ImGui::PlotHistogram(const char* label, float (*values_getter)(void* tex_data, int idx), void* tex_data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    PlotEx(ImGuiPlotType_Histogram, label, values_getter, tex_data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}
